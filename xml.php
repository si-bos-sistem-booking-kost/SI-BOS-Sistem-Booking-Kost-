...
if ($_GET["searched"] == "true") {
  $sql .= " WHERE status='$_GET[status]' AND nama LIKE '%$_GET[nama]%' AND harga_1hari >= $_GET[min] AND harga_1bulan <= $_GET[max]";
...
while ($row = $query->fetch_assoc()){
  ...
  $newnode->setAttribute("harga_1hari", $row['harga_1hari']);
  $newnode->setAttribute("harga_1bulan", $row['harga_1bulan']);
